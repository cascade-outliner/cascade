...
## 2026-07-23
- [fix] Fixed the filters dropdown briefly getting covered by the tree whenever applying/changing a filter reflowed the rows underneath it. The filter change navigates via a URL search-param update, which was picking up the app's default view transition — meant only for the row-to-detail-page shared-element animation — and using it to animate every row's reflow in the browser's view-transition overlay, which paints above everything else including the open dropdown. View transitions are now opt-in per navigation instead of on-by-default, so only the row-to-detail navigation uses one. [#402](https://github.com/Patrickroelofs/cascade/issues/402)
