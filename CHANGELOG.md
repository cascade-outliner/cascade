...
## 2026-08-05
- [perf] Clicking a node's focus dot no longer flashes to a blank page while the detail view loads — the authed app shell was re-checking the session and re-fetching settings/premium status over the network on every single in-app navigation (not just fresh page loads), which added enough latency to outlast the cross-fade transition. The session check is now cached briefly instead of re-run on every click; the detail route also gained a lightweight fallback for the rare case its own render still needs a moment, and its code is now warmed proactively as soon as the tree loads rather than waiting for a hover that a first click or touch tap has no chance to trigger. [#635](https://github.com/cascade-outliner/cascade/issues/635)
