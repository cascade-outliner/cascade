...
- [feat] Version history no longer shows a line-by-line content diff — every entry now renders as a read-only recreation of the node's content exactly as it looks in the outliner (rich text, headings, due-date and tag pills), and a whole-subtree deletion entry shows the entire deleted subtree the same way, indentation included. That preview is now virtualized too, so a large deleted subtree doesn't mount hundreds of rows at once. [#408](https://github.com/Patrickroelofs/cascade/issues/408)
