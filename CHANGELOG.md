...
## 2026-08-07
- [fix] The due-date picker's day grid is now announced correctly to screen readers — it's marked up as a real table with column headers, each day button has a full-date label and reflects its selected/today state via `aria-pressed`/`aria-current`, and the "select an end date" hint is announced live instead of only appearing visually. [#657](https://github.com/cascade-outliner/cascade/issues/657)
