...
- [feat] Redesigned the app header: a floating bar with the existing breadcrumb trail (moved here from the node detail page, not duplicated) on wider screens, and a thumb-reachable bottom action dock (Home, Agenda, Quick Open, User menu) on narrow screens instead of squeezed-in top icons. On narrow screens, the Agenda panel and Quick Open dialog now float with rounded corners, a margin from the screen edges, and the same near-full-screen height, without the top of the header bar covering their own title/close button; the dock stays visible and usable the whole time — opening one from the dock while the other is open switches between them instead of stacking.
