...
## 2026-08-06
- [feat] A board-converted node now renders its board inline wherever it appears in an outline — the root outline included — instead of only on its own detail page: expanding it shows the status-grouped card columns in place of nested tree rows, collapsing it hides them again. A board's expand toggle is now available even when it has no cards yet (instead of being suppressed like a childless row's would be), and converting a node into a board expands it right away so the board — even an empty one — is visible immediately instead of still looking like plain text. Board cards now have their own dedicated drag handle instead of the whole card being draggable, so on touch devices the rest of a card — tapping to edit its title, and its "Convert into"/duplicate/delete context menu, the same one a tree row has — is reachable again instead of being swallowed by drag; the handle itself sits outside that context menu, so long-pressing/right-clicking it only starts a drag and doesn't also pop the menu. The board's cards now respect the outline's active filters (tags, priority, status, due date, hide-completed) the same way tree rows do — previously a board always showed every one of its direct children regardless of the outline's filter bar. Board cards are now full nodes like any other: they share the tree row's own icon, due date, priority, status, and tags — including the pickers/editors for each, from the card's own context menu — instead of only showing a read-only priority pill and tag pills with no way to set a due date, icon, or status from the board. [#455](https://github.com/cascade-outliner/cascade/issues/455)
