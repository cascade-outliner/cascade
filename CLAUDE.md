...
pnpm perf:reconcile:app # benchmark TanStack Query mutation-reconciliation cost (response -> cache write -> paint)
...
- `pnpm perf:reconcile:app` (`mutation-reconciliation.spec.ts`, also run by `pnpm test:perf:ui:app`) measures the one dimension the rest of the harness doesn't: the cost of TanStack Query reconciling a mutation's server response into the committed UI, independent of network latency and of the DOM/animation timing `virtual-tree-motion.spec.ts` measures (see #597). It seeds a small scratch subtree, then drives the real "create below" flow (Enter after editing a row) repeatedly in a real browser — the one node mutation (`useCreateMutation` in `use-create-node.ts`) that patches its query cache only after the server responds rather than optimistically in `onMutate` like the other node mutations, so there's a genuine response-to-store gap to measure. `recordMutationReconciliation` (`use-create-node.ts` → `mutation-reconciliation-perf.ts`) takes a `performance.now()` mark right after the mutation resolves and another right after the matching cache write, then a double-`requestAnimationFrame` mark for the next paint; it's inert unless `window.__CASCADE_MUTATION_TIMING__` is set (by this spec), so it costs real users nothing. Reports `reconciliation` (response → cache write), `paint` (cache write → next frame), and `total` latency percentiles to `apps/web-app/perf-results/` (`reconciliation-bench.json`).
...
- `virtual-tree-motion.spec.ts` (also run by `pnpm test:perf:ui:app`, needs `perf:seed:app --count=20000` already run first) is the issue #509 rollout gate for tree-row motion: it seeds a dedicated scratch subtree per action (expand/collapse toggle, create-below, delete, drag reorder) as a root-level node inside the already-seeded 20,000-node tree, then drives each action in a real browser with the candidate single-row WAAPI reposition effect (`animateRowReposition`, gated by `window.__CASCADE_ROW_MOTION__`/`isRowMotionEnabled()` in `packages/outliner/src/tree/components/virtual-tree-row.tsx` — unset for real users) toggled on and off, plus once under `page.emulateMedia({ reducedMotion: "reduce" })`. Each run asserts mounted `role="treeitem"` count stays bounded regardless of motion state and records action latency + requestAnimationFrame gap stats to `apps/web-app/perf-results/motion-bench.json`. Tree-motion child issues under #507 are gated on this passing.
- `.github/workflows/perf.yml` runs `perf:seed`/`perf:query`/`perf:mutate`/`perf:filter`/`perf:workflow` against both a PR's base and head commit and posts a single comment on the PR with the `visibleTree`, `createNode`/`moveNode`/`duplicateNode`, filter (`tagFilter`/`dueTodayFilter`), and combined-workflow (`fullWorkflow`) before/after comparison. It's a comparison for a human to read, not a merge gate, and only triggers on changes to `apps/web-app`/`packages/outliner`/`packages/auth`/`packages/http`; it does not run the Playwright `test:perf:ui:app` project (including `virtual-tree-motion.spec.ts` and `mutation-reconciliation.spec.ts`), which stays a manually-run gate for now.
