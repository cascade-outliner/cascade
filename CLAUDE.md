...
- `packages/auth` — better-auth setup (`createAuth(db)`), used by `apps/web-app` and `apps/api`; the resulting session cookie is scoped to span both origins in production (`COOKIE_DOMAIN`). Unlike every other shared package, its `exports` point at a real `tsc` build (`dist/`, via `pnpm build:auth`) rather than raw TypeScript source — `apps/api` runs its production output with a bare `node dist/main.js`, which can't resolve or transpile another workspace package's `.ts` source the way Vite's dev server does for `apps/web-app`/`apps/website`. `pnpm install`'s `postinstall` hook (and `build:app`/`build:web`/`build:api`) build it automatically; editing `packages/auth/src` needs a `pnpm build:auth` rerun before the change is visible to any consumer.
