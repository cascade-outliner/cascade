...
- `.github/workflows/perf-ui.yml` runs `pnpm test:perf:ui:app` (including `virtual-tree-motion.spec.ts`) automatically on the same path filters as `perf.yml`: it seeds the 20,000-node tree, then lets `playwright.perf.config.ts`'s `webServer` build and start the app before the spec projects run. Unlike `perf.yml`'s comparison comment, this job fails outright when the spec's assertions fail (bounded mounted `role="treeitem"` count, per-action latency/frame-gap collection), so — once branch protection marks it required — it's an actual merge gate rather than a manually-run one.
