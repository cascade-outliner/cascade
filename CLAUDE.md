...
pnpm perf:mutate:app    # benchmark createNode/moveNode latency
...
- `pnpm perf:mutate:app -- --creates=50 --moves=50` benchmarks `createNode` and `moveNode` (advisory-locked reorder + fractional-index recomputation) latency over real HTTP calls, same server/auth requirements as `perf:query:app`. Both operations run against two scratch parent nodes created just for the run (not the seeded tree from `perf:seed:app`), so results don't depend on `--shape`/`--count` and repeated runs don't drift the seeded tree's size; the scratch parents (and everything created under them) are deleted via `deleteNode`'s cascade when the run finishes. Results also go to `apps/app/perf-results/` (`mutation-bench.json`).
- `pnpm perf:report:app -- --beforeDir=<dir> --afterDir=<dir>` diffs two `perf-results/` snapshots (`query-bench.json` and, if present, `mutation-bench.json`) into a markdown comparison table.
...
- `.github/workflows/perf.yml` runs `perf:seed`/`perf:query`/`perf:mutate` against both a PR's base and head commit and posts a single comment on the PR with the `visibleTree`, `createNode`, and `moveNode` before/after comparison. It's a comparison for a human to read, not a merge gate, and only triggers on changes to `apps/app`/`packages/outliner`/`packages/auth`/`packages/http`.
- **Not yet measured, and why**: filter application (toggling date ranges/tags/search) is pure client-side logic in `packages/outliner`, not an HTTP call the harness can benchmark without a browser; drag-and-drop latency (mouse-up to rendered result) and mutation reconciliation (server response to visible-tree update in TanStack Query) are both client-side/browser timings, closer in kind to `test:perf:ui:app`'s Playwright approach than to the HTTP-only `query-bench.ts`/`mutation-bench.ts` scripts — none of the three have a harness yet.
