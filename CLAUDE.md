...
`createNode` inserts its own marker row (`created: true`, `content: null`) the moment a node is made, so a brand new, never-edited node shows up in `listTreeVersions` immediately as "Node created" — non-restorable, since there's no prior state to go back to. `listNodeVersions` (a specific node's own history) excludes this marker, so a node's own version history stays genuinely empty until its first real edit, rather than starting with a confusing "created but nothing to show or restore" entry.

`deleteNode` also inserts one `node_versions` marker row for the directly-targeted node only (`descendantsDeleted`, the count of descendants deleted alongside it — `NULL` on every normal content-edit row). `listTreeVersions` shows this marker but hides every other version belonging to a currently-deleted node (except the `created` marker, which keeps showing regardless — deleting a node later doesn't erase the record of when it came into being), so a whole-subtree deletion collapses into a single tree-wide history entry instead of one per affected node; `listNodeVersions` (a specific node's own history) is unaffected and still lists everything. Restoring a marker (`created` or `descendantsDeleted`) skips `snapshotAndSetContent` entirely — neither ever touched `content`, so there's nothing to roll back there; a `descendantsDeleted` marker still has `deletedAt`/`parentId`/`order` to undo, a `created` marker has nothing to undo at all (the UI disables Restore for it).
